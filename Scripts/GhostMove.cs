...
public class GhostMove : MonoBehaviour {
	...
	void FixedUpdate ()
	{
		...
		if (Random.value < 0.1f) {
			int num2;
			string posx = ""+transform.position.x;
			string posy = ""+transform.position.y;
			
			if(int.TryParse(posx,out num2) && int.TryParse(posy,out num2) ){
				DropPackdot();
			}	
		}
		*/
	void Update(){
		...
		else if(currentscene.name == "infrequent"){
			if (Random.value < 0.02f) {
			int num3;
			string posx = ""+transform.position.x;
			string posy = ""+transform.position.y;
			
				if(int.TryParse(posx,out num3) && int.TryParse(posy,out num3) ){
					DropPacdot();
				}	
			}
		}
...
void DropPacdot()
{
    ...
    if (canDrop)
    {
        Instantiate(pacdot, transform.position, Quaternion.identity);
    }
}
